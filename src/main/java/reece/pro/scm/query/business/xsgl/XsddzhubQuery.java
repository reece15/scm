...
public class XsddzhubQuery extends BaseQuery {
	...
	public Map<String, Object> buildWhere() {
		if (StringUtils.isNotBlank(ddh)) {
			this.getQueryMap().put("ddh", ddh);
		}
		if (dhrq != null) {
			this.getQueryMap().put("dhrq", dhrq);
		}
		if (sxrq != null) {
			this.getQueryMap().put("sxrq", sxrq);
		}
		if (StringUtils.isNotBlank(khmc)) {
			this.getQueryMap().put("khmc", khmc);
		}
		if (zdkl != null) {
			this.getQueryMap().put("zdkl", zdkl);
		}
		if (StringUtils.isNotBlank(sxbm)) {
			this.getQueryMap().put("sxbm", sxbm);
		}
		if (StringUtils.isNotBlank(ywy)) {
			this.getQueryMap().put("ywy", ywy);
		}
		if (StringUtils.isNotBlank(kpdw)) {
			this.getQueryMap().put("kpdw", kpdw);
		}
		if (StringUtils.isNotBlank(shdq)) {
			this.getQueryMap().put("shdq", shdq);
		}
		if (StringUtils.isNotBlank(shdz)) {
			this.getQueryMap().put("shdz", shdz);
		}
		if (StringUtils.isNotBlank(spr)) {
			this.getQueryMap().put("spr", spr);
		}
		if (sprq != null) {
			this.getQueryMap().put("sprq", sprq);
		}
		if (StringUtils.isNotBlank(zdr)) {
			this.getQueryMap().put("zdr", zdr);
		}
		if (zdrq != null) {
			this.getQueryMap().put("sprq", sprq);
		}
		if (StringUtils.isNotBlank(xgr)) {
			this.getQueryMap().put("xgr", xgr);
		}
		if (xgrq != null) {
			this.getQueryMap().put("xgrq", xgrq);
		}
		if(state!=null){//销售订单状态
			this.getQueryMap().put("state",state);
		}
		if(thbz!=null){//退货标志
			this.getQueryMap().put("thbz",thbz);
		}
		if(ckjsbz!=null){//出库结束标志
			this.getQueryMap().put("ckjsbz",ckjsbz);
		}
		if(kpjsbz!=null){//开票结束标志
			this.getQueryMap().put("kpjsbz",kpjsbz);
		}
		if(fhjsbz!=null){//发货结束标志
			this.getQueryMap().put("fhjsbz",fhjsbz);
		}
		if(zdjshj!=null){//整单税价合计
			this.getQueryMap().put("zdjshj",zdjshj);
		}
		if(sxkje!=null){//收现款金额
			this.getQueryMap().put("sxkje",sxkje);
		}
